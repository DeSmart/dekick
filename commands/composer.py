...
def main(parser: Namespace, args: list):  # pylint: disable=unused-argument
    ...
    if parser.log_filename:
        ...
        exit_code = ui_composer(args=args)
    else:
        exit_code = composer(args=args, raise_exception=False, capture_output=False)[
            "returncode"
        ]
...
def ui_composer(**kwargs):
    """UI wrapper for docker_compose"""

    def wrapper(**kwargs):
        composer(raise_exception=True, **kwargs)

    args = kwargs["args"][0] if "args" in kwargs else ""

    return run_func(
        text=f"Running composer {C_CMD}{args}{C_END}", func=wrapper, func_args=kwargs
    )
...
def composer(
    ...
    """It runs composer in a container
    ...
    cmd = "run"
    args = ["--rm", "--user", CURRENT_UID, container, "composer"] + args
